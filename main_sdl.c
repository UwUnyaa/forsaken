...
{
#if SDL_VERSION_ATLEAST(2, 0, 0) && defined(GL)
	(void)renderer_flags;
#endif
	...
	if (SDL_GL_SetSwapInterval(render_info.vsync ? 1 : 0) != 0)
		DebugPrintf("main_sdl: unable to set swap interval: %s\n", SDL_GetError());
#else
...
bool sdl_init_video(void)
{
	...
	u_int32_t renderer_flags = 0;
#ifndef GL
	renderer_flags = create_renderer_flags();
#endif
#else
